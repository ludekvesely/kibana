...
if [ "$1" = 'kibana' ]; then
	if [ "$ELASTICSEARCH_URL" -o "$ELASTICSEARCH_PORT_9200_TCP" ]; then
		...
		sed -i \
      -e "s#-KIBANA_INDEX-#${KIBANA_INDEX}#" \
      -e "s#-KIBANA_ELASTICSEARCH_USERNAME-#${KIBANA_ELASTICSEARCH_USERNAME}#" \
      -e "s#-KIBANA_ELASTICSEARCH_PASSWORD-#${KIBANA_ELASTICSEARCH_PASSWORD}#" \
